...
def inv_pm_app():
    ...
    with test_app.app_context():
        ...
        # Wire invoice use-cases with payment_method_repo
...
def _make_auto_generated_release_row(app, project_id: str, payment_method_id=None, label=None):
    """Insert an auto-generated released_funds InvoiceModel row directly.

    Mirrors what SetInvoiceRefundableStatusUseCase's bank-refund path creates —
    is_auto_generated=True, type=released_funds — so the PUT carve-out
    (payment_method_id-only edits) has a route-reachable auto-generated row to
    exercise against, without wiring the full refund/funds-release use case.
    """
    from app import db
    ...
    with app.app_context():
        row = InvoiceModel(
            ...
            items=[{"description": "Bank refund release", "quantity": 1, "unit_price": 100, "vat_rate": 0}],
...
class TestAutoGeneratedReleaseCarveOutRoute:
    def test_put_payment_method_id_only_on_auto_generated_release_succeeds(
        self, inv_pm_client, inv_pm_app, admin_token
    ):
        """Route-level happy path for the narrow carve-out: PUT with ONLY
        payment_method_id on an auto-generated released_funds invoice returns 200
        and changes the attribution — exercises the exclude_unset sentinel
        plumbing between the route and UpdateInvoiceUseCase, which the
        application-layer carve-out unit tests (against fakes) do not cover."""
        old_pm_id = _make_pm_row(inv_pm_app, inv_pm_app._test_company_a_id, label="Old Attribution")
        new_pm_id = _make_pm_row(inv_pm_app, inv_pm_app._test_company_a_id, label="New Attribution")
        invoice_id = _make_auto_generated_release_row(
            inv_pm_app, inv_pm_app._test_project_id, payment_method_id=old_pm_id, label="Old Attribution"
        )

        resp = inv_pm_client.put(
            _invoice_url(inv_pm_app._test_project_id, invoice_id),
            json={"payment_method_id": new_pm_id},
            headers=_auth(admin_token),
        )
        assert resp.status_code == 200, resp.get_data(as_text=True)
        data = resp.get_json()
        assert data["payment_method_id"] == new_pm_id
        assert data["payment_method_label"] == "New Attribution"

    def test_put_any_other_field_on_auto_generated_release_still_rejected(self, inv_pm_client, inv_pm_app, admin_token):
        """Baseline still holds at the route level: an edit that includes a
        non-payment_method_id field disqualifies the carve-out and returns 403."""
        invoice_id = _make_auto_generated_release_row(inv_pm_app, inv_pm_app._test_project_id)

        resp = inv_pm_client.put(
            _invoice_url(inv_pm_app._test_project_id, invoice_id),
            json={"recipient_name": "Renamed"},
            headers=_auth(admin_token),
        )
