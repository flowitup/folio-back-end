...
def test_cash_advance_row_is_labelled_as_such():
    """The summary row says 'Others (cash advance)' so it is not mistaken for an expense."""
    ctx = _make_context()
    advance = _make_invoice(amount=Decimal("500.00"), recipient="Advance Holder").with_updates(is_cash_advance=True)
    other = _make_invoice(invoice_type=InvoiceType.OTHERS, amount=Decimal("40.00"), recipient="Divers")
    wb = openpyxl.load_workbook(BytesIO(build_xlsx(ctx, _make_bundle([advance, other]))))
    rows = [r for r in wb["Summary"].iter_rows(values_only=True) if r and r[3] in ("Advance Holder", "Divers")]
    labels = {r[3]: r[2] for r in rows}
    assert labels == {"Advance Holder": "Others (cash advance)", "Divers": "Others"}
