...
class LastCompanyAdminError(Exception):
...
class DeleteAccountResult:
    """What was erased, for the audit log."""

    user_id: UUID
    detached_companies: int
...
class DeleteAccountUseCase:
    ...
    def execute(self, user_id: UUID) -> DeleteAccountResult:
        ...
        self._reject_if_last_admin_of_a_shared_company(user_id, accesses)

        self._users.save(user.anonymized(self._clock.now()))
        self._eraser.erase_for_user(user_id)
        self._db.commit()

        return DeleteAccountResult(user_id=user_id, detached_companies=len(accesses))
    ...
    def _reject_if_last_admin_of_a_shared_company(
        ...
        for access in accesses:
            ...
            if not other_members:
                continue
            raise LastCompanyAdminError(
                access.company_id, self._company_name(access.company_id)
            )

    def _company_name(self, company_id: UUID) -> str:
        company: Optional[object] = self._companies.find_by_id(company_id)
        return getattr(company, "legal_name", "") or str(company_id)
