"""MarkNoteOpenUseCase — transition a note back to status='open'."""
...
class MarkNoteOpenUseCase:
    """Reopen a note (set status back to 'open').
    ...
    def execute(self, *, actor_id: UUID, note_id: UUID) -> NoteDto:
        """Transition note to 'open'; return updated DTO.
        ...
        open_note = note.mark_open()
        self._note_repo.save(open_note)
        self._db.commit()
        return NoteDto.from_entity(open_note)
